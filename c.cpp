...
or the smallest value in the right subtree based off a random value
***************/
...
void BinaryTree::RemoveMatch(node* parent, node* found, bool left)
{
	...
	if(root != NULL)
	{
		...
		else
		{
			...
			smallestInRightSubtree = FindSmallestPrivate(found->right); //finds smallest in tree
			RemoveNodePrivate(smallestInRightSubtree, found); //deletes the smallest node in subtree
			found->data = smallestInRightSubtree; //replaces the current data with the smallest in right tree
			}
...
void BinaryTree::RemoveRootMatch()
{
	...
	if(root != NULL) //if tree  is empty
	{
		...
		{			
			rnum = rand() % 2;
			if(rnum == 0){
			largestInLeftSubtree = FindLargestPrivate(root->left); //finds smallest in tree
			RemoveNodePrivate(largestInLeftSubtree, root); //deletes the smallest node in subtree
			root->data = largestInLeftSubtree; //replaces the current data with the smallest in right tree
			//cout << "The root data containing data " << rootData << " was overwritten with data " << root->data << endl;
			}
			else{
			smallestInRightSubtree = FindSmallestPrivate(root->right); //finds smallest in tree
			RemoveNodePrivate(smallestInRightSubtree, root); //deletes the smallest node in subtree
			root->data = smallestInRightSubtree; //replaces the current data with the smallest in right tree
			//cout << "The root data containing data " << rootData << " was overwritten with data " << root->data << endl;
			}
...
void BinaryTree::AddLeafPrivate(int data, node* Ptr)
{
	...
	else if(data > Ptr->data) //if data is greater than current value
	{
		if(Ptr->right != NULL)
		{
			AddLeafPrivate(data, Ptr->right); //recursively moves down the left pointer
		}
...
int main()
{
	...
	while(1){
	cout << "\n1: (50 runs) Insert default input sequence then delete each individually from the root: 46,21,62,16,34,49,75,3,20,24,42,48,60,70,96\n2: Add Your Own Leaf\n3: Remove Leaf\n4: Print inorder of tree\nOther: exit program" << endl;
	cout << "\nEnter your choice: ";
	cin >> menu;
	switch(menu)
	{
		case '1':
			for(int j=0;j<50;j++){ //Makes 50 runs of the insertions and deletions to display the total number of recursions made for each of them to test the randomness accurately
			for(int i=0;i<15;i++)
			{
			...
			cout << "Total number of function calls including recursions for run " << j+1 << ": " << tree.recCount << endl;
			runavg+=tree.recCount;
			tree.recCount=0;
			}
			runavg=runavg/50;
			cout << "\nAverage number of function calls: " << runavg << endl << endl;
			return 0;
			break;
		case '2': 
